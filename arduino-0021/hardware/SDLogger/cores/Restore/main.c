...
#define STAT1_DDR   DDRB

//STAT2 is a general LED and indicates volume SPI traffic
#define STAT2  1
#define STAT2_PORT  PORTB
#define STAT2_DDR   DDRB
...
void read_config_file(void)
{
  ...
  if(find_file_in_dir(fs, dd, config_file_name, &file_entry, 0))
  {
    ...
    if(system_baud == BAUD_38400) strcpy(temp_string, "38400");
    if(system_baud == BAUD_57600) strcpy(temp_string, "57600");
    if(system_baud == BAUD_115200) strcpy(temp_string, "115200");
    if(system_baud == BAUD_230400) strcpy(temp_string, "23040000");
...
uint8_t append_file(char* file_name)
{
  ...
  //They get stored in one big array 2048 wide. As the array fills, we store one quater the array (512 bytes) at a time.
