...
#define SOCKWP		0x08			/* Write protect switch (PC3) */
#define SOCKINS		0x04			/* Card detect switch (PC2) */
...
void power_on (void)
{
	PORTB = 0b10111100;			/* Enable drivers */
	DDRB  = 0b10110011;
...
void power_off (void)
{
	...
	DDRB  = 0b00000011;		/* Disable drivers */
	PORTB = 0b00001100;
...
void disk_timerproc (void)
{
    ...
    if (pv & SOCKWP)	{ // Write protected 
      s |= STA_PROTECT;
      PORTB |= 0x02; // turn on red led
    } else { //Write enabled
      s &= ~STA_PROTECT;
      PORTB &= 0xfd; // turn off red led
    }
    if (pv & SOCKINS)	{ // Socket empty  
      s |= (STA_NODISK | STA_NOINIT);
      PORTB |= 0x01; // turn on green led
    } else { // Card inserted
      s &= ~STA_NODISK;
      PORTB &= 0xfe; // turn off green led
    }
    Stat = s;
  }
